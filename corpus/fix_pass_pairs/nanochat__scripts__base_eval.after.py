...
def main():
    ...
    if 'core' in eval_modes:
        ...
        core_metric_result = core_results["core_metric"]
        core_task_results = core_results["results"]
        core_centered_results = core_results["centered_results"]
        ...
        if ddp_rank == 0:
            ...
            with open(output_csv_path, 'w', encoding='utf-8', newline='') as f:
                ...
                for label in core_task_results:
                    acc = core_task_results[label]
                    centered = core_centered_results[label]
                    f.write(f"{label:<35}, {acc:<10.6f}, {centered:<10.6f}\n")
                f.write(f"{'CORE':<35}, {'':<10}, {core_metric_result:<10.6f}\n")
            print0(f"\nResults written to: {output_csv_path}")
            print0(f"CORE metric: {core_metric_result:.4f}")
    ...
    if core_results:
        report_data[0]["CORE metric"] = core_metric_result
        report_data.append(core_centered_results)
