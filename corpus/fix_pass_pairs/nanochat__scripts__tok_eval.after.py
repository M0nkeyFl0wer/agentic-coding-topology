...
for tokenizer_name in ["gpt2", "gpt4", "ours"]:
    ...
    vocab_sizes[tokenizer_name] = tokenizer_vocab_size
    tokenizer_test_results = {}
    tokenizer_results[tokenizer_name] = tokenizer_test_results
    ...
    for name, text in all_text:
        ...
        tokenizer_test_results[name] = text_stats
...
def print_comparison(baseline_name, baseline_results, ours_results, all_text):
    ...
    for name, text in all_text:
        ...
        relative_diff = ((baseline_tokens - ours_tokens) / baseline_tokens) * 100

        # Determine which has better compression (higher ratio = better)
        if baseline_ratio > ours_ratio:
            baseline_color, ours_color = GREEN, RED
            better = baseline_name
            diff_color = RED
        elif ours_ratio > baseline_ratio:
        ...
        print(f"{name:<10} {baseline_bytes:<8} "
              f"{baseline_color}{baseline_tokens:<7}{RESET} "
              f"{baseline_color}{baseline_ratio:<7.2f}{RESET} "
              f"{ours_color}{ours_tokens:<7}{RESET} "
              f"{ours_color}{ours_ratio:<7.2f}{RESET} "
...
for baseline_name in ["GPT-2", "GPT-4"]:
    ...
    for name, text in all_text:
        baseline_data = baseline_results[name]
        baseline_data_bytes = baseline_data['bytes']
        baseline_data_tokens = baseline_data['tokens']
        baseline_data_ratio = baseline_data['ratio']
        
        ours_data = ours_results[name]
        ours_data_tokens = ours_data['tokens']
        ours_data_ratio = ours_data['ratio']
        
        relative_diff = ((baseline_data_tokens - ours_data_tokens) / baseline_data_tokens) * 100
        lines.append(f"| {name} | {baseline_data_bytes} | {baseline_data_tokens} | {baseline_data_ratio:.2f} | {ours_data_tokens} | {ours_data_ratio:.2f} | {relative_diff:+.1f}% |")
    lines.append("")
report_markdown = "\n".join(lines)
